...
function preload(){
    //loadImages
    flappyImg = loadImage("/assets/galo.png");
    //baixo
...
function mecanica(){ 
    //gravidade
    var maxSpeed = 10;
    if(player.velocityY < maxSpeed){
        player.velocityY += maxSpeed/10;
    }

    //pulo
    if(keyDown("down") && pressed === false){
        player.velocityY = maxSpeed*3.5;
        pressed = true;
    }
    if(keyWentUp("down")){
        pressed = false;
    ...
    //dash
    if(keyDown("right")){
        player.velocityY = (maxSpeed-4)/2;
    }
}

function colisao(){
    //bordas
    if(player.isTouching(upEdge) || player.isTouching(obstacles)){
        //resetar score
        lastScore = score;
        score = 0;
        biomeCounter = 0;
