...
function preload(){
    //loadImages
    flappyImg = loadImage("/assets/vaca.png");
    //baixo
...
function mecanica(){
    ...
    if(keyDown('right') && pressed === false){
        if(direcao === "baixo"){
            direcao = "cima";
            pressed = true;
            return;
...
function colisao(){
    ...
    player.collide(upEdge);
    player.collide(downEdge);
}

function criarObstaculos(){
    var randomvar = Math.round(random(-windowHeight+70,-70));

    if(score%60 === 0){
        var obstacle  = createSprite(windowWidth+10,randomvar                             ,50,windowHeight*2);
        var obstacle2 = createSprite(windowWidth+10,randomvar+windowHeight-20+windowHeight+60,50,windowHeight*2);
